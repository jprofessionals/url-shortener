...
use domain::{
    AuditAction, AuditEntry, AuditRepository, ClickEvent, ClickRepository, CoreError, GroupMember,
...
impl DynamoTables {
    ...
    pub fn from_env() -> Result<Self, CoreError> {
        ...
        let groups = std::env::var("DYNAMO_TABLE_GROUPS").unwrap_or_else(|_| "Groups".into());
        let group_members =
            std::env::var("DYNAMO_TABLE_GROUP_MEMBERS").unwrap_or_else(|_| "GroupMembers".into());
        let clicks = std::env::var("DYNAMO_TABLE_CLICKS").unwrap_or_else(|_| "Clicks".into());
        let audit = std::env::var("DYNAMO_TABLE_AUDIT").unwrap_or_else(|_| "AuditLog".into());
        Ok(Self {
            shortlinks,
            counters,
            groups,
...
impl DynamoRepo {
    ...
    pub fn increment_global_counter(&self) -> Result<u64, CoreError> {
        ...
        let attrs = out
            .attributes()
            .ok_or_else(|| CoreError::Repository("update returned no attributes".into()))?;
        let v = attrs
            .get("value")
            .and_then(|av| av.as_n().ok())
            .ok_or_else(|| CoreError::Repository("counter missing value".into()))?;
        v.parse::<u64>()
            .map_err(|e| CoreError::Repository(format!("parse counter: {e}")))
    }
}
...
impl LinkRepository for DynamoRepo {
    ...
    fn list(&self, limit: usize) -> Result<Vec<ShortLink>, CoreError> {
        ...
        let fut = async { self.client.scan().table_name(table).limit(lim).send().await };
        let out = self.block_on(fut).map_err(map_sdk_err)?;
        let mut res = Vec::new();
        for it in out.items().iter() {
            if let Ok(sl) = item_to_domain(it) {
                res.push(sl);
            }
        }
        Ok(res)
    ...
    fn list_paginated(&self, options: &ListOptions) -> Result<ListResult<ShortLink>, CoreError> {
        ...
        if let Some(ref email) = options.created_by {
            ...
            expr_values.insert(
                ":email".into(),
                AttributeValue::S(email.as_str().to_string()),
            );
        }
        ...
        if let Some(ref q) = options.search {
            filter_parts.push(
                "(contains(#slug, :q) OR contains(original_url, :q) OR contains(description, :q))"
                    .to_string(),
            );
        ...
        let out = self.block_on(fut).map_err(map_sdk_err)?;
        let mut all_items: Vec<ShortLink> = out
            .items()
            .iter()
            ...
            let fut = async {
                self.client
                    ...
                    .expression_attribute_values(
                        ":ts",
                        AttributeValue::N(deleted_at_secs.to_string()),
                    )
                    .condition_expression(
                        "attribute_exists(slug) AND attribute_not_exists(deleted_at)",
                    )
                    .send()
                    .await
            };
...
fn domain_to_item(link: &ShortLink) -> HashMap<String, AttributeValue> {
    ...
    m.insert(
        "slug".into(),
        AttributeValue::S(link.slug.as_str().to_string()),
    );
    m.insert(
        "original_url".into(),
        AttributeValue::S(link.original_url.clone()),
    );
    m.insert(
        "created_at".into(),
        AttributeValue::N(system_time_to_secs(link.created_at).to_string()),
    );
    m.insert(
        "created_by".into(),
        AttributeValue::S(link.created_by.as_str().to_string()),
    );
    m.insert(
        "click_count".into(),
        AttributeValue::N(link.click_count.to_string()),
    );
    m.insert("is_active".into(), AttributeValue::Bool(link.is_active));
    if let Some(updated_at) = link.updated_at {
        m.insert(
            "updated_at".into(),
            AttributeValue::N(system_time_to_secs(updated_at).to_string()),
        );
    }
    if let Some(expires_at) = link.expires_at {
        m.insert(
            "expires_at".into(),
            AttributeValue::N(system_time_to_secs(expires_at).to_string()),
        );
    }
    ...
    if let Some(activate_at) = link.activate_at {
        m.insert(
            "activate_at".into(),
            AttributeValue::N(system_time_to_secs(activate_at).to_string()),
        );
    }
    if let Some(redirect_delay) = link.redirect_delay {
        m.insert(
            "redirect_delay".into(),
            AttributeValue::N(redirect_delay.to_string()),
        );
    }
    if let Some(deleted_at) = link.deleted_at {
        m.insert(
            "deleted_at".into(),
            AttributeValue::N(system_time_to_secs(deleted_at).to_string()),
        );
    }
...
fn item_to_domain(item: &HashMap<String, AttributeValue>) -> Result<ShortLink, CoreError> {
    let slug = item
        ...
        .ok_or_else(|| CoreError::Repository("item missing slug".into()))?;
    let original_url = item
        .get("original_url")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("item missing original_url".into()))?
        .to_string();
    let created_at = item
        .get("created_at")
        .and_then(|v| v.as_n().ok())
        .ok_or_else(|| CoreError::Repository("item missing created_at".into()))?;
    let created_by = item
        .get("created_by")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("item missing created_by".into()))?;
    ...
    let description = item
        ...
        .and_then(|v| v.as_s().ok())
        .map(|s| s.to_string());
    let activate_at = item
        .get("activate_at")
        .and_then(|v| v.as_n().ok())
        .and_then(|s| s.parse::<u64>().ok())
        .map(secs_to_system_time);
    let redirect_delay = item
        .get("redirect_delay")
        .and_then(|v| v.as_n().ok())
    ...
    let slug = Slug::new(slug.to_string())
        .map_err(|e| CoreError::Repository(format!("bad slug in item: {e}")))?;
...
fn group_to_item(group: &LinkGroup) -> HashMap<String, AttributeValue> {
    ...
    m.insert(
        "created_at".into(),
        AttributeValue::N(system_time_to_secs(group.created_at).to_string()),
    );
    m.insert(
        "created_by".into(),
        AttributeValue::S(group.created_by.as_str().to_string()),
    );
...
fn item_to_group(item: &HashMap<String, AttributeValue>) -> Result<LinkGroup, CoreError> {
    let id = item
        .get("id")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("group missing id".into()))?
        .to_string();
    let name = item
        .get("name")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("group missing name".into()))?
        .to_string();
    let created_at = item
        .get("created_at")
        .and_then(|v| v.as_n().ok())
...
fn member_to_item(member: &GroupMember) -> HashMap<String, AttributeValue> {
    ...
    m.insert(
        "group_id".into(),
        AttributeValue::S(member.group_id.clone()),
    );
    m.insert(
        "user_email".into(),
        AttributeValue::S(member.user_email.as_str().to_string()),
    );
    m.insert(
        "role".into(),
        AttributeValue::S(member.role.as_str().to_string()),
    );
    m.insert(
        "added_at".into(),
        AttributeValue::N(system_time_to_secs(member.added_at).to_string()),
    );
    m.insert(
        "added_by".into(),
        AttributeValue::S(member.added_by.as_str().to_string()),
    );
    m
}
...
fn item_to_member(item: &HashMap<String, AttributeValue>) -> Result<GroupMember, CoreError> {
    let group_id = item
        .get("group_id")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("member missing group_id".into()))?
        .to_string();
    let user_email = item
        .get("user_email")
        .and_then(|v| v.as_s().ok())
...
fn click_to_item(event: &ClickEvent) -> HashMap<String, AttributeValue> {
    ...
    m.insert(
        "slug".into(),
        AttributeValue::S(event.slug.as_str().to_string()),
    );
    m.insert(
        "clicked_at".into(),
        AttributeValue::N(system_time_to_secs(event.clicked_at).to_string()),
    );
...
fn item_to_click(item: &HashMap<String, AttributeValue>) -> Result<ClickEvent, CoreError> {
    ...
    let slug =
        Slug::new(slug.to_string()).map_err(|e| CoreError::Repository(format!("bad slug: {e}")))?;
...
fn audit_to_item(entry: &AuditEntry) -> HashMap<String, AttributeValue> {
    ...
    m.insert(
        "timestamp".into(),
        AttributeValue::N(system_time_to_secs(entry.timestamp).to_string()),
    );
    m.insert(
        "actor_email".into(),
        AttributeValue::S(entry.actor_email.as_str().to_string()),
    );
    m.insert(
        "action".into(),
...
fn item_to_audit(item: &HashMap<String, AttributeValue>) -> Result<AuditEntry, CoreError> {
    let id = item
        .get("id")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("audit missing id".into()))?
        .to_string();
    let timestamp = item
        .get("timestamp")
        .and_then(|v| v.as_n().ok())
    ...
    let target_type = item
        .get("target_type")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("audit missing target_type".into()))?
        .to_string();
    let target_id = item
        .get("target_id")
        .and_then(|v| v.as_s().ok())
        .ok_or_else(|| CoreError::Repository("audit missing target_id".into()))?
        .to_string();
    let changes = item
        .get("changes")
        .and_then(|v| v.as_s().ok())
...
mod tests {
    ...
    fn roundtrip_item_mapping() {
        ...
        assert_eq!(
            system_time_to_secs(link.created_at),
            system_time_to_secs(link2.created_at)
        );
    ...
    fn backward_compatible_item_mapping() {
        ...
        item.insert(
            "original_url".into(),
            AttributeValue::S("https://old.com".into()),
        );
        item.insert("created_at".into(), AttributeValue::N("1700000000".into()));
        item.insert(
            "created_by".into(),
            AttributeValue::S("user@example.com".into()),
        );
